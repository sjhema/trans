...
<c:set var="weeklySalaryRateAlertListPg" value="${payrollRateAlertListPg}?type=weeklySalaryRate"/>
<table border="0" style="margin-left: 20px; margin-top: 20px;">
	<c:if test="${expiredDriverPayRateList == null
			...
			&& expiredWeeklySalaryRateList == null}">
		<tr>
			<td style="color:grey;">
				<h3>
					No alerts.
	...
	<c:if test="${(weeklySalaryRateExprdCount != null && weeklySalaryRateExprdCount ne 0)
				|| (weeklySalaryRateExprngCount != null && weeklySalaryRateExprngCount ne 0)}">
		<jsp:include page="alertInfo.jsp">
			<jsp:param name="alertType" value="Hourly rate(s)"/>
		    <jsp:param name="expiredCount" value="${weeklySalaryRateExprdCount}"/>
		    <jsp:param name="expiringCount" value="${weeklySalaryRateExprngCount}"/>
		    <jsp:param name="alertListPg" value="${expiredWeeklySalaryRateList}"/>
		</jsp:include>
	</c:if>
</table>