...
<h3><primo:label code="Manage Holiday Type"/></h3>
<div><form:form action="search.do" method="get"
	name="searchForm">
	<table id="form-table" width="100%" cellspacing="0" cellpadding="5">
		<tr class="table-heading">
			<td align="${left}" colspan="9"><b><primo:label code="Search Holiday Type"/></b></td>
		</tr>
		<tr>
			<td align="${left}" class="first"><primo:label code="Name"/></td>
				<td align="${left}"><select id="name" name="id" style="min-width:154px; max-width:154px">
					...
					<c:if test="${sessionScope.searchCriteria.searchMap['id'] == holidaytype.id}">
						<c:set var="selected" value="selected"/>
					</c:if>
						<option value="${holidaytype.id}" ${selected}>${holidaytype.name}</option>
					</c:forEach>
			</select></td>
		...
		<div style="width:100%; margin:0px auto;"><form:form name="delete.do" id="holidayForm">
		<primo:datatable urlContext="hr/holiday" deletable="true" editable="true" insertable="true" exportPdf="true" exportXls="true" exportCsv="true"
			baseObjects="${list}"
			searchCriteria="${sessionScope['searchCriteria']}" cellPadding="2"
			pagingLink="search.do" multipleDelete="false" searcheable="false" >
				<primo:textcolumn headerText="Holiday" dataField="name"/>	
