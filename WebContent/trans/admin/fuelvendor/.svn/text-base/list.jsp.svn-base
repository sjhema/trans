...
<h3>
	<primo:label code="Manage Fuel Vendors" />
</h3>
<form:form action="search.do" method="get" name="searchForm">
	<table width="100%" id="form-table">
		<tr class="table-heading">
			<td colspan="4"><b><primo:label code="Search Fuel Vendors" /></b></td>
		</tr>
		<tr>
			<td align="${left}" class="first"><primo:label code="Name"/></td>
			<td align="${left}"><select id="fuelvendor" name="id" style="min-width:200px; max-width:200px">
				<option value="">------<primo:label code="Please Select"/>------</option>
				<c:forEach items="${fuelvendor}" var="fuelvendor">
					<c:set var="selected" value=""/>
					<c:if test="${sessionScope.searchCriteria.searchMap['id'] == fuelvendor.id}">
...
<form:form name="delete.do" id="serviceForm">
	<primo:datatable urlContext="admin/fuelvendor" deletable="true"
