CREATE DEFINER=`lutransport`@`%` PROCEDURE `SubcontractorInvoice3`(IN `rate_transfer_id` BIGINT(150), IN `rate_landfill_id` BIGINT(150), IN `rate_subcontractor_id` BIGINT(150))
BEGIN
...
voucher_date,voucher_status,gallon from ticket where voucher_status=1 and origin=rate_transfer_id and destination=rate_landfill_id and subcontractor_id=rate_subcontractor_id;
...
SELECT id into subcont_id from subcontractor_invoice_detail_new where tickt_id=ticket_id;
