CREATE DEFINER=`lutransport`@`%` PROCEDURE `BillingHistoryProc5`()
BEGIN
...
DECLARE invoice_dates_var varchar(225);
...
landfill_net,transfer_net,entered_by,company_location,

voucher_date,voucher_status,gallon,invoice_date from ticket where ticketstatus=2 and bill_batch>='2013-03-01 00:00:00' and bill_batch<='2013-04-30 00:00:00';
...
landfill_nets,transfer_nets,entered_bys,compnay_locations,

voucher_dates,voucher_statuss,gallons,invoice_dates;
...
rate,tonnage_premium,total_amount,bill_gross,bill_net,bill_tare,bill_tons,invoice_number

into FAmount,DemmurageCharge,fuelSurcharge,Minimumbillablegrossweight,FRate,

TonnagePremium,TOtAmount,EffectiveGrossWt,EffectiveNetWt,EffectiveTareWt,EffectiveTonsWt,invoice_numbers

from invoice_detail where ticket_id=tickets_id LIMIT 0,1;
...
SET invoice_dates_var=DATE_FORMAT(invoice_dates,'%m-%d-%Y');
...
fuelSurcharge,gallons,invoice_dates_var,invoice_numbers,landfill_time_ins,landfill_time_outs,Load_date_var,Minimumbillablegrossweight,
...
OriginName,UnitNumber,TerminalName,tickets_id,TonnagePremium,TotAmount,TrailerNumber,transfer_time_ins,tranfer_time_outs,Unload_date_var,null,destinations,origins,drivercompany_name,drivercompany_id,unload_dates,driverPayRate);
