CREATE DEFINER=`lutransport`@`%` PROCEDURE `BillingHistoryProc1`(IN rate_transfer_id bigint(150))
BEGIN
...
voucher_date,voucher_status,gallon from ticket where (ticketstatus=1 OR ticketstatus=0) and origin=rate_transfer_id and destination in (253,254);
