CREATE DEFINER=`lutransport`@`%` PROCEDURE `BillingHistoryProc3`(IN rate_transfer_id bigint(150),IN rate_landfill_id bigint(150))
BEGIN
...
DECLARE EffectiveGrossWt double;
...
voucher_date,voucher_status,gallon from ticket where (ticketstatus=1 OR ticketstatus=0) and origin=rate_transfer_id and destination=rate_landfill_id;
...
BLOCK2: BEGIN
    ...
    rate_using,customer_name,billed_by,sort_by,fuel_surcharge_weeklyrate ,customer_name
    ...
    FETCH frstCursor into compnay_locations,rate_types,
    ...
    rate_usings,customer_names,billed_bys,sort_bys,fuel_surcharge_weeklyrates,customer_ids;
...
update invoice_detail_new set amount=FAmount,billing_using=fBill_using,bill_date=bill_batch_date_var,tcompany=Company_Name,bill_customer_name=Customer_Names,demmurage_charge=DemmurageCharge,dest_gross=landfill_grosss,

dest_net=landfill_nets,dest_tare=landfill_tares,destination_ticket=destination_tickets,dest_tons=landfill_tons,t_driver=DriverName,bill_gross=EffectiveGrossWt,bill_net=EffectiveNetWt,bill_tare=EffectiveTareWt,bill_tons=EffectiveTonsWt,t_enteredby=EnteredByName,

fuel_surcharge=fuelSurcharge,gallon_num=gallons,invoiced_date=invoice_dates,bill_invoice_number=invoice_numbers,landfill_time_in=landfill_time_ins,landfill_time_out=landfill_time_outs,loaded=Load_date_var,minimum_billablewt=min_billable_gross_weights,

origin_gross=transfer_grosss,origin_net=transfer_nets,origin_tare=transfer_tares,origin_ticket=origin_tickets,origin_tons=transfer_tons,process_status=ProcessStatus,rate=rvalues,bill_subcontractor=SubcontractorName,t_destination=DestinationName,

t_origin=OriginName,t_unit=UnitNumber,t_terminal=TerminalName,tickt_id=ticket_id,tonnage_premium=TonnagePremium,total_amount=TotAmount,trailer_num=TrailerNumber,transfer_time_in=transfer_time_ins,transfer_time_out=tranfer_time_outs,unloaded=Unload_date_var,fuel_padd_code=PaddCode,t_destination_id=destinations,t_origin_id=origins,driver_companylocname=drivercompany_name,driver_companylocid=drivercompany_id,unload_date=unload_dates,driver_pay_rate=driverPayRate
