...
public class ImportMainSheetServiceImpl implements ImportMainSheetService {
	...
	private static Logger log = Logger
			.getLogger(ImportMainSheetServiceImpl.class.getName());
	   ...
	   // List<String> emptydatalist=new ArrayList<String>();
		...
		try 
		{
						...
						{					  
							...
							if(tollNum!=null )
							{
								...
								{					  
									...
									StringBuffer query=new StringBuffer("select obj from VehicleTollTag obj where obj.tollTagNumber='"+(String)getCellValue(row.getCell(3))+"'") ;   
									...
									query.append(" and obj.validFrom <='"+ transactiondate+ "' and obj.validTo >= '"+transactiondate+"'");
									...
									{	
										String vehquery="Select obj from Vehicle obj where obj.unit="+vehicletolltags.get(0).getVehicle().getUnit()+" and obj.validFrom <='"+ transactiondate+ "' and obj.validTo >= '"+transactiondate+"'";
										...
										if(!(StringUtils.isEmpty(drv_name))){											
											...
											String drivequery="select obj from Ticket obj where   obj.loadDate<='"+transactiondate+"' and obj.unloadDate>='"+transactiondate+"' and obj.vehicle="+vehicle.get(0).getId();
											...
											if(!tickets.isEmpty()){
												...
												if(tic){
													...
													Driver driver=genericDAO.getById(Driver.class,tickets.get(0).getDriver().getId());
													eztoll.setTerminal(driver.getTerminal());
												}	
											}else{
												String driverFuelLogQuery="select obj from DriverFuelLog obj where   obj.transactionDate='"+transactiondate+"' and obj.truck="+vehicle.get(0).getId();

												System.out.println(" my query is "+driverFuelLogQuery);
												List<DriverFuelLog> driverFuelLog=genericDAO.executeSimpleQuery(driverFuelLogQuery);
												...
												if(tic){
													...
													Driver driver=genericDAO.getById(Driver.class,driverFuelLog.get(0).getDriver().getId());
													eztoll.setTerminal(driver.getTerminal());
												}
											}
											else{	
												
												String driverOdometerQuery="select obj from Odometer obj where obj.recordDate='"+transactiondate+"' and obj.truck="+vehicle.get(0).getId();

												System.out.println(" odometer query is "+driverOdometerQuery);
												List<Odometer> odometer=genericDAO.executeSimpleQuery(driverOdometerQuery);
												...
												for(Odometer odometerObj:odometer){
													...
													boolean d=driverid.contains(odometerObj.getDriver().getId()+"");
												...
												if(tic){
													...
													Driver driver=genericDAO.getById(Driver.class,odometer.get(0).getDriver().getId());
													eztoll.setTerminal(driver.getTerminal());
												}
												...
												if(override==false){
												error = true;
												lineError.append("No matching  Ticket, Fuel Log,  Odometer entry, ");
												}
												else{
													try 
												    {
								...
								try 
								{
									...
									{		
										...
										if(tollNum!=null)
										{
											...
											query.append(" and obj.vehicle='"+vehicle.getId()+"' and obj.validFrom <='"+ transactiondate+ "' and obj.validTo >= '"+transactiondate+"'");							
											
											/*String query="select obj from VehicleTollTag obj where obj.tollTagNumber='"
											+(String)getCellValue(row.getCell(3))+"' and obj.vehicle='"
											+vehicle.getId()+"' and obj.validFrom <='"
											+ transactiondate+ "' and obj.validTo >= '"
											+transactiondate+"'";*/		
											...
											{	
												...
												List<Vehicle> vehicleList = genericDAO.executeSimpleQuery("select o from Vehicle o where o.unit="+vehicletolltags.get(0).getUnit()+" and o.validFrom<=SYSDATE() and o.validTo>=SYSDATE() ");
									    ...
									    else
									    {
											...
											if (validDate(getCellValue(row.getCell(6))))
											{								
												transactiondate1=dateFormat.format(((Date) getCellValue(row.getCell(6))).getTime());
												System.out.println("\n****--****\n");
											}
									    	
									    	StringBuffer query=new StringBuffer("select obj from VehicleTollTag obj where ");
									    	query.append("obj.vehicle='"+vehicle.getId()+"' and obj.validFrom <='"+ transactiondate1+ "' and obj.validTo >= '"+transactiondate1+"'");
									    	if(eztoll.getToolcompany()!=null){
												query.append(" and obj.tollCompany='"+eztoll.getToolcompany().getId()+"'");
											}
											...
											if(!(StringUtils.isEmpty(drv_name))){											
												...
												String drivequery="select obj from Ticket obj where   obj.loadDate<='"+transactiondate+"' and obj.unloadDate>='"+transactiondate+"' and obj.vehicle="+vehicle.getId();
												...
												if(!tickets.isEmpty()){
													...
													String driverFuelLogQuery="select obj from DriverFuelLog obj where   obj.transactionDate='"+transactiondate+"' and obj.truck="+vehicle.getId();
													...
													String driverOdometerQuery="select obj from Odometer obj where obj.recordDate='"+transactiondate+"' and obj.truck="+vehicle.getId();
							...
							else
							{
								if(eztoll.getTollTagNumber()!=null)
								{
									...
									/* 	Code to get the active plate numbers*/
									List<Vehicle> vehicleList = genericDAO.executeSimpleQuery("select o from Vehicle o where o.unit="+vehicletoll.getUnit()+" and o.validFrom<=SYSDATE() and o.validTo>=SYSDATE() ");
									if (vehicleList.isEmpty()&& vehicleList.size()==0)
										throw new Exception("Invalid Plate Number");
									else
										eztoll.setPlateNumber(vehicleList.get(0));
					 ...
					 //FOR TRANSACTION TIME
					    ...
					    if(!(StringUtils.isEmpty(trxTime1))){
					    	if (trxTime1.length() == 5 || trxTime1.length() == 8||trxTime1.length() == 7){
				    	         StringBuilder time = new StringBuilder(StringUtils.leftPad((String)getCellValue(row.getCell(7)),4,'0'));
				    	         //time.insert(2, ':');
           ...
           /* if (validTime((String) getCellValue(row.getCell(6))))
            {
                        StringBuilder time = new StringBuilder(StringUtils.leftPad((String)getCellValue(row.getCell(6)),4,'0'));
						...
						{					  
							...
							if(tollNum!=null )
							{
								...
								{					  
									...
									System.out.println(" Size of vehicle is "+vehicletolltags.size());
									if (vehicletolltags.isEmpty()&& vehicletolltags.size()==0)
										throw new Exception("no such Toll tag Number");
									else
									{	
										...
										if(!(StringUtils.isEmpty(drv_name))){											
											...
											if(!tickets.isEmpty()){
												...
												if(override==false){
												error = true;
												lineError.append("Ticket Data Does not match with Toll Tag, ");
												}
												else{
													try 
												    {
            ...
            else
            {
                error = true;
					  ...
					  //END OF CELL
						...
						if (!error) 
						{
							java.sql.Date transacDt = new java.sql.Date(eztoll.getTransactiondate().getTime()); 
							try {
								StringBuffer query = new StringBuffer();
								query.append("select obj from EzToll obj ");
								query.append(" where obj.toolcompany.name='"+tollcompany+"'");
								query.append(" and obj.company.name='"+company+"' ");
								if(tollNum != null)
								...
								List<EzToll> duplicateCheck = genericDAO.executeSimpleQuery(query.toString());
								
								if(duplicateCheck!= null && duplicateCheck.size()>0){
									
									error=true;
									errorcount++;
									lineError.append("Toll Tag already exists, ");
								}
   ...
   return list;
	...
	public List<String> importfuellogMainSheet(InputStream is,Boolean override) throws Exception {
				...
				try {
						...
						try {
							if (StringUtils.isEmpty(Fname)){
								...
								if (fuelvendor == null){
									...
									//throw new Exception("no such fuelvender");
								}
								else{
					...
					/*try
					    {
					      if (validDate(getCellValue(row.getCell(5))))
							  fuellog.setTransactiondate((Date) getCellValue(row.getCell(5)));
						  else 
						  {
							   error = true;
							   lineError.append("transaction date,");
							...
							if(!(StringUtils.isEmpty(trxTime1))){
								if(override==false){
									if (trxTime1.length() == 5 || trxTime1.length() == 8){
										StringBuilder time = new StringBuilder(StringUtils.leftPad((String)getCellValue(row.getCell(5)),4,'0'));
										//time.insert(2, ':');
										if(trxTime1.length() == 8){
						...
						try 
						{
							if (StringUtils.isEmpty(unit)){
								...
								if (driver == null){
									...
									String drivequery="select obj from Ticket obj where   obj.loadDate<='"+transdate+"' and obj.unloadDate>='"+transdate+"' and obj.driver="+driver.getId();
									...
									if(!tickets.isEmpty()){
										...
										String driveFuelLogquery="select obj from DriverFuelLog obj where  obj.transactionDate='"+transdate+"' and obj.driver="+driver.getId();
										...
										if(!driverFuelLog.isEmpty()){
											...
											String odometerQery="select obj from Odometer obj where obj.recordDate='"+transdate+"' and obj.driver="+driver.getId();
											...
											if(!driverOdometer.isEmpty()){
												...
												error = true;
												lineError.append("Unit is blank, No matching  Ticket, Fuel Log,  Odometer entry, ");								
											}
										}
									}											
								}
								...
								String vehicleQuery = "Select obj from Vehicle obj where obj.type=1 and obj.unit="+Integer
								.parseInt((String) getCellValue(row
										.getCell(6)))+" and obj.validFrom<='"+transactionDate+"' and obj.validTo>='"+transactionDate+"'";
								...
								if(vehicleList==null || vehicleList.size()==0)
								{
									...
									//taking driverFname and driverLName and storing as fullname
									try{  
										if (StringUtils.isEmpty(lastName) && StringUtils.isEmpty(firstName) ){
											...
											if(!tickets.isEmpty()){
												...
												String driveFuelLogquery="select obj from DriverFuelLog obj where   obj.transactionDate='"+transactiondate+"' and obj.truck="+vehicle.getId();
												System.out.println("********driver fuel query is "+driveFuelLogquery);
												List<DriverFuelLog> driverFuelLog=genericDAO.executeSimpleQuery(driveFuelLogquery);
												if(!driverFuelLog.isEmpty()){
													boolean tic=true;
													...
													String odometerQery="select obj from Odometer obj where obj.recordDate='"+transactiondate+"' and obj.truck="+vehicle.getId();
													System.out.println("********odometer query is "+odometerQery);
													List<Odometer> driverOdometer=genericDAO.executeSimpleQuery(odometerQery);
													...
													if(!driverOdometer.isEmpty()){
														...
														List<String> driverid=new ArrayList<String>();
														for(Odometer odometer:driverOdometer){
															boolean d=driverid.contains(odometer.getDriver().getId()+"");
														...
														if(tic){
															...
															Driver driver=genericDAO.getById(Driver.class,driverOdometer.get(0).getDriver().getId());
															fuellog.setTerminal(driver.getTerminal());												
														}	
													}
													else{
					...
					/*if(override==false)
							    {
							      try 
							      {
									//fuellog.setFuelCardNumber(((String) getCellValue(row.getCell(10))));
							    	  if(!cardNo.isEmpty()){
								            fuellog.setFuelCardNumber((String)getCellValue(row.getCell(10)));
								      }
							    	  else{
							...
							if(!StringUtils.isEmpty(cardNo)){
								...
								if(!fuelcard.isEmpty() && fuelcard.size()>0 ){	
									if(fuellog.getDriversid()!=null && fuellog.getFuelvendor()!=null){						    			
										...
										List<DriverFuelCard> driverfuelcard=genericDAO.findByCriteria(DriverFuelCard.class, criterias);							    									    			 
										if(!driverfuelcard.isEmpty() && driverfuelcard.size()>0)							    			
											fuellog.setFuelcard(fuelcard.get(0));							    			 
										else {							    				
											error = true;
											lineError.append(" Invalid  Fuel Card# for entered Fuel Vendor and Driver ,");
										}
									}						    			  
								}
								else {	
					...
					/*String cardNo = ((String) getCellValue(row.getCell(10)));
					    if(override==false)
					    {
					      try 
					      {
							fuellog.setFuelCardNumber(Long.parseLong((String) getCellValue(row.getCell(10))));
					    	  if(!cardNo.isEmpty()){
						            fuellog.setFuelCardNumber((String)getCellValue(row.getCell(10)));
					    		  fuellog.setFuelCardNumber((Long)getCellValue(row.getCell(10)));
						      }
					    	  else{
					    		    error = true;
									lineError.append("Card Number is blank,");
					...
					/*String testgallon=row.getCell(14).toString();
							    Double gallon = getValidGallon(testgallon);
							    if (gallon != null)
								{
								  fuellog.setGallons(gallon);
								}
								else 
								{
									lineError.append("gallons,");
									error = true;
								}*/
					...
					//for unitprice 
					/*String unitprice=row.getCell(15).toString();
							   Double unitprice1 = getValidGallon(unitprice);
							   if (unitprice1 != null)
								{
     								fuellog.setUnitprice(unitprice1);
								}
								else 
								{
									System.out.println("\nunitprice is null\n");
									lineError.append("unitprice,");
									error = true;
								}*/
					String unitprice1="";
					if(override==false)
					...
					//FOR AMOUNTS
					/*String amount1=row.getCell(18).toString();
							   Double amount2 = getValidGallon(amount1);
							   if (amount2 != null){
					...
					//CALCULATING DISCOUNT AND NET AMMOUNT IF FUELDISCOUNT PERCENTAGE IS PRESENT	
					if(override==false)
					{
						if (!error) {
							try{	
								...
								if (grossamount == null ){
									...
									if(discountAmount ==0 && feesAmount ==0){
										//double grossAmount=NetAmount+(discountAmount-feesAmount);
		...
		if (errorcount==0) 
		{
			...
			{
				String ticktQuery = "select obj from Ticket obj where obj.driver="+fuelog.getDriversid().getId()+" and obj.loadDate <='"+drvdf.format(fuelog.getTransactiondate())+"' and obj.unloadDate>='"+drvdf.format(fuelog.getTransactiondate())+"'";
			...
			while (rows.hasNext()) {
				...
				try {
					...
					if (validDate(getCellValue(row.getCell(0))))
						ticket.setLoadDate((Date) getCellValue(row
								.getCell(0)));
					else {
						error = true;
					...
					if (validTime((String) getCellValue(row.getCell(1)))) {
						StringBuilder timeIn = new StringBuilder(StringUtils.leftPad((String) getCellValue(row
								.getCell(1)),4,'0'));
						timeIn.insert(2, ':');
					...
					try {
						...
						criterias.put("unit", Integer
								.parseInt((String) getCellValue(row
										.getCell(3))));
						Vehicle vehicle = genericDAO.getByCriteria(
								Vehicle.class, criterias);
						if (vehicle == null)
					...
					try {
						criterias.clear();
						criterias.put("type",1);
						criterias.put("name", (String) getCellValue(row
								.getCell(8)));
					...
					try {
						ticket.setOriginTicket(Long
								.parseLong((String) getCellValue(row
										.getCell(9))));
					} catch (Exception ex) {
						error = true;
					...
					try
					{
						...
						{
							//BillingRate billingRate = null;
							String  query="select obj from BillingRate obj where transferStation="+originid.getId()+" and landfill="+destinationid.getId();
							...
							if(billingRate != null){
							ticket.setCompanyLocation((billingRate.getCompanyLocation()!=null)?billingRate.getCompanyLocation():null);
							ticket.setCustomer((billingRate.getCustomername()!=null)?billingRate.getCustomername():null);
							}
							else
							{
							...
							/*{
								error = true;
								lineError.append("Rate is expired for this origin and destination,please contact to administrator,");
							}*/
						}
				  ...
				  try {
						ticket.setDestinationTicket(Long
								.parseLong((String) getCellValue(row
										.getCell(11))));
					} catch (Exception ex) {
						error = true;
				...
				if(ticket.getDestination()!=null){
					...
					Double tgross = getValidAmount((String) getCellValue(row
							.getCell(12)));
					if (tgross != null)
					...
					Double ttare = getValidAmount((String) getCellValue(row
							.getCell(13)));
					if (ttare != null)
					...
					Double lgross = getValidAmount((String) getCellValue(row
							.getCell(16)));
					if (lgross != null)
					...
					Double ltare = getValidAmount((String) getCellValue(row
							.getCell(17)));
					if (ltare != null)
					...
					String driverName = ((String) getCellValue(row
							.getCell(21)));
					Driver driver = null;
					try {
						if (StringUtils.isEmpty(driverName))
							...
							driver = genericDAO.getByCriteria(Driver.class,
									criterias);
							if (driver == null)
								throw new Exception("Invalid driver");
					...
					try {
						String employeeCompanyName = ((String) getCellValue(row
								.getCell(23)));
						if (StringUtils.isEmpty(employeeCompanyName))
							...
							Location employeeCompany = genericDAO.getByCriteria(Location.class,
									criterias);
					...
					String subcontractor = ((String) getCellValue(row
							.getCell(20)));
					try {
						if (!StringUtils.isEmpty(subcontractor)) {
							/*if (driver != null
								...
								SubContractor contractor = genericDAO
										.getByCriteria(SubContractor.class,
												criterias);
								if (contractor == null) {
									throw new Exception(
					...
					try {
						criterias.clear();
						String locCode = (String) getCellValue(row
								.getCell(24));
					...
					try {
						User user = genericDAO.getByUniqueAttribute(
								User.class, "username",
	...
	try {
		...
		while (rows.hasNext()){
			...
			if(!error){
				...
				String query;
				try {
					query = "select obj from VehiclePermit obj "
						+" where obj.vehicle.unitNum='"+unitNum+"'"
						+" and obj.companyLocation.name='"+company+"' "
						+" and obj.companyLocation.type=3 "
						+" and obj.permitType="+setupData.getId()+" "
						...
						+" or obj.issueDate>='"+issueDate+" 00:00:00' AND obj.expirationDate<='"+expirationDate+" 00:00:00')";
						//+" and obj.issueDate='"+dateFormat1.format(date4)+"' "
						//+" and obj.expirationDate='"+dateFormat1.format(date5)+"'";
